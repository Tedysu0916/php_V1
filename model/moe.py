...
class MoeLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, task_param) -> torch.Tensor:
        ...
        # 每个专家的最大容量 (在forward中计算)
        # capacity = int(inputs.shape[0] * inputs.shape[1] * self.args.num_experts_per_tok * 1.2 / self.args.num_experts)

        # 限制每个专家处理的tokens数量
        # for i, expert in enumerate(self.experts):
        #     idx_1, idx_2, nth_expert = torch.where(selected_experts == i)
        #     # 如果超过容量，随机丢弃
        #     if idx_1.size(0) > capacity:
        #         perm = torch.randperm(idx_1.size(0), device=idx_1.device)
        #         idx_1 = idx_1[perm[:capacity]]
        #         idx_2 = idx_2[perm[:capacity]]
        #         nth_expert = nth_expert[perm[:capacity]]
