...
class IRRA(nn.Module):
    def __init__(self, args, num_classes=11003):
        ...
        self.shared_transformerv = MOETransformer(self.embed_dim, layers, heads, attn_mask=None, num_experts=args.num_experts, topk=args.topk, reduction=args.reduction)
        self.shared_transformert = MOETransformer(self.embed_dim, layers, heads, attn_mask=None, num_experts=args.num_experts, topk=args.topk, reduction=args.reduction)
        # parameter init
        for i in range(layers):
            for j in range(args.num_experts):
                nn.init.kaiming_uniform_(self.shared_transformerv.resblocks[i].feed_forward.experts[j].down.weight,
                                         a=math.sqrt(5))
                nn.init.zeros_(self.shared_transformerv.resblocks[i].feed_forward.experts[j].down.bias)
                nn.init.zeros_(self.shared_transformerv.resblocks[i].feed_forward.experts[j].up.weight)
                nn.init.zeros_(self.shared_transformerv.resblocks[i].feed_forward.experts[j].up.bias)

        for i in range(layers):
            for j in range(args.num_experts):
                nn.init.kaiming_uniform_(self.shared_transformert.resblocks[i].feed_forward.experts[j].down.weight,
                                         a=math.sqrt(5))
                nn.init.zeros_(self.shared_transformert.resblocks[i].feed_forward.experts[j].down.bias)
                nn.init.zeros_(self.shared_transformert.resblocks[i].feed_forward.experts[j].up.weight)
                nn.init.zeros_(self.shared_transformert.resblocks[i].feed_forward.experts[j].up.bias)
        ...
        self.saflv = SAFL(dim=self.embed_dim, part_num=self.num,)
        self.saflt = SAFL(dim=self.embed_dim, part_num=self.num, )
    ...
    def encode_image(self, image):
        ...
        safl_image_feats = self.saflv(x[:, 1:, :])
        safl_image_feats, _ = self.shared_transformerv(safl_image_feats, 0)
        i_feats = x[:, 0, :].float()
    ...
    def encode_text(self, text):
        ...
        safl_text_feats = self.saflt(x)
        safl_text_feats, _ = self.shared_transformert(safl_text_feats, 0)
    ...
    def forward(self, batch):
        ...
        safl_image_feats = self.saflv(image_feats[:, 1:, :])
        safl_text_feats = self.saflt(text_feats)
        ...
        safl_image_feats, l_aux_v = self.shared_transformerv(safl_image_feats, 0)
        safl_text_feats, l_aux_t = self.shared_transformert(safl_text_feats, 0)
        #
