...
logger = logging.getLogger("PHP.model")
...
class MOEResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, num_experts=6, topk=2, reduction=8, i=None):
        super().__init__()
        ...
        if self.index >= 0:
            self.feed_forward = MoeLayer(
                experts=[Adapter(d_model, d_model, reduction,0.1,True) for _ in range(self.num_experts)],
    ...
    def forward(self, para_tuple: tuple):
        ...
        if self.index >= 0:
            ...
            y, l_aux = self.feed_forward(self.ln_3(x), self.comb_param.to(x.dtype))
            # y, l_aux = self.feed_forward(self.ln_3(x), self.task_param)
...
class MOETransformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, num_experts=6, topk=2, reduction=8):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[MOEResidualAttentionBlock(width, heads, attn_mask, num_experts, topk, reduction, i) for i in range(layers)])
